...
#+begin_src emacs-lisp
  ...
  (use-package :emacs
    :ensure nil
    :bind (:map rc/custom-bindings-map
                ("C-z C-f" . rc/toggle-fold)))
#+end_src

*** =rc/term=

Launch =ansi-term= using the preferred shell (=rc/preferred-shell=).

#+begin_src emacs-lisp
  (defcustom rc/preferred-shell
    "/bin/bash"
    "Default shell to use with `rc/term`."
    :type 'string)

  (defun rc/term (&optional new-buffer-name)
    "Launch `ansi-term` using RC/PREFERRED-SHELL."
    (interactive)
    (ansi-term rc/preferred-shell new-buffer-name))
#+end_src
