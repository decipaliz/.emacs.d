...
I tried using [[info:eglot][eglot]], but in the end, decided to switch back to =lsp-mode=. I tend to use languages with language servers that are difficult to install on the system, and I also prefer the fancy =lsp-ui=, glances instead of a separate buffer and the breadcrumbs. Maybe some day I will have much free time and I will implement these features using =eglot=, but not today.
