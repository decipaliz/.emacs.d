...
This is my attempt at simplifying my existing GNU Emacs configuration that’s
been used for several years. I’ve encountered that I tend to create many complex
file structures inside of my configuration and also don’t document things as
fervently as one would like.

Both of these problems are addressed by using a configuration file in [[info:org#Top][Org mode]].
It lets you keep all settings in one file and naturally encourages you to
document them. Even if the file might get very long, navigating it inside Org
mode is simple and convenient.

* Configuration

This is an Org file which is [[info:org#Extracting Source Code][tangled]] into actual configuration. Whenever you see
a source code block like this:
...
Tangling the =init.org= file every time manually isn’t satisfactory. We can
define a function and add it to =after-save-hook= to do it automatically every
time =init.org= changes.
...
These modes are global - meaning once they are enabled, they work by default in
every buffer.
...
I prefer the Hack font, and I also prefer to use a fixed pitch font in place of
variable pitch font too. You can download Hack Nerd Font from [[https://www.nerdfonts.com/font-downloads][Nerd Fonts]].
...
To avoid garbage in the filesystem, I prefer to place autosaves in a special
directory.

#+begin_src emacs-lisp
  (defvar deci/autosave-directory
    (expand-file-name "autosave/" user-emacs-directory)
    "The directory where autosave files should be put.")

  (setq backup-directory-alist
        `((".*" . ,deci/autosave-directory))
        auto-save-file-name-transforms
        `((".*" ,deci/autosave-directory t)))
#+end_src
...
I use a minor mode that has an associated keymap to hold all of my custom
keybindings.
...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    :ensure nil
    :bind (:map deci/custom-bindings-map
                ("C-z C-\\" . deci/insert-lambda)))
#+end_src

*** =deci/toggle-fold=

Toggle fold no matter what folding mode is enabled (=hs= or =outline-indent=)

#+begin_src emacs-lisp
  (defun deci/toggle-fold ()
    "Toggle fold no matter what folding mode is enabled."
    (interactive)
    (cond
     (outline-indent-minor-mode (outline-indent-toggle-fold))
     (hs-minor-mode (hs-toggle-hiding))))

  (use-package :emacs
    :ensure nil
    :bind (:map deci/custom-bindings-map
                ("C-z C-f" . deci/toggle-fold)))
#+end_src
...
amount of extra dependencies (because there’s a *lot* of them in my older
configuration). Thankfully, [[info:use-package#Top][use-package]] is integrated into Emacs since Emacs 29,
so I can use this convenience without feeling bad about installing it.
...
I was debating between using the default Emacs modeline and the Doom modeline
and decided to use the [[https://github.com/rougier/nano-modeline][N Λ N O Modeline]] instead. Maybe I'll define my own
modeline later to get rid of this package too.
...
Despite using Emacs for quite a while, I still find this plugin extremely
useful.
...
I have been addicted to [[https://joaotavora.github.io/yasnippet/][YASnippet]] for quite a while now. Can't imagine using
Emacs without it.
...
Despite Projectile having support of ripgrep, I still like to be able to
manually search in directories using =rg=.
...
A minor mode for Emacs that does folding based on indentation.
...
I tried using [[info:eglot][eglot]], but in the end, decided to switch back to =lsp-mode=. I
tend to use languages with language servers that are difficult to install on the
system, and I also prefer the fancy =lsp-ui=, glances instead of a separate
buffer and the breadcrumbs. Maybe some day I will have much free time and I will
implement these features using =eglot=, but not today.
...
I like to use a sort of a "local leader" for language-specific keybindings, this
local leader is =C-z C-c= for me. Unfortunately, I don't think there's a way to
set this sort of thing concisely. Fortunately, Emacs Lisp has macros.

#+begin_src emacs-lisp
    (defmacro deci/local-map (hook &rest pairs)
...
#+begin_src emacs-lisp
  (use-package cider
    :hook (((clojure-mode cider-mode) . hs-minor-mode)
           ((clojure-mode cider-mode) . lsp-deferred)))

  (deci/local-map clojure-mode-hook
...
#+begin_src emacs-lisp
  (use-package dockerfile-mode
    :hook ((dockerfile-mode . outline-indent-minor-mode)))
  (use-package docker-compose-mode
    :hook ((docker-compose-mode . outline-indent-minor-mode)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (deci/local-map racket-mode-hook
...
#+begin_src emacs-lisp
  (use-package renpy
    :vc (renpy-mode :url "https://github.com/Reagankm/renpy-mode.git"
                    :branch "master")
    :hook ((renpy-mode . outline-indent-minor-mode)))
#+end_src
...
This configuration is implying that [[http://gambitscheme.org/][Gambit Scheme]] is used.
...
#+begin_src emacs-lisp
  ...
  (use-package scheme-mode
    ...
    :init
    (setq scheme-program-name "gsi -:d@ gambit/geiser -"))

  (deci/local-map scheme-mode-hook
...
#+begin_src emacs-lisp
  (use-package uiua-mode
    :mode "\\.ua\\'"
    :hook (uiua-mode . (lambda ()
                         (interactive)
                         (setq buffer-face-mode-face '(:family "Uiua386" :height 200))
                         (buffer-face-mode))))

  (defun deci/uiua-run ()
    "Run current buffer with Uiua."
    (interactive)
    (shell-command (format "%s run %s" uiua-command (buffer-file-name))))

  (defun deci/uiua-test ()
    "Test current buffer with Uiua."
    (interactive)
    (shell-command (format "%s test %s" uiua-command (buffer-file-name))))

  (deci/local-map uiua-mode-hook
                ("C-r" . deci/uiua-run)
                ("C-t" . deci/uiua-test)
                ("C-f" . uiua-format-buffer)
                ("C-b" . uiua-standalone-compile))
#+end_src
...
#+begin_src emacs-lisp
  (use-package vue-mode
    :hook ((vue-mode . outline-indent-minor-mode)
